...
本 skill が守るべき不変条件。違反は実行中断条件。詳細は「アンチパターン」セクションも参照。

### MUST（必ず守る）

- **AGENTS.md を実体、CLAUDE.md は AGENTS.md への symlink**：cross-agent 標準と Claude Code 互換性を両立させる（ステップ 6）
- **50 行以内（推奨）/ 200 行以下（上限）**：超えたら短縮する。長文化は遵守率を急落させる（ステップ 5）
- **ポインタ先の実在確認**：書く前に Glob で存在確認、書いた後も検証（ステップ 2 / 7）
- **コードから導けない規約のみ本文に書く**：`ls` / Makefile / `go.mod` / `package.json` などで分かることは参照で済ます
- **既存独自規約は消さない**：更新モードでユーザー固有のルールを意図せず削除しない（ステップ 1）

### FORBIDDEN（絶対にやらない）

- **存在しないファイルへのポインタ**：「予定」「想定」のファイルは書かない（腐敗検出が壊れる）
- **ディレクトリ構造の文章解説**：`ls` で分かることを本文に書かない
- **ビルド手順 / コマンドの羅列**：Makefile / package.json への参照で済ませる
- **動的状態（WIP / 進行中タスク / カバレッジ %）**：AGENTS.md は静的ドキュメント、動的情報は PR / issue へ
- **絶対パスやユーザー固有環境変数**：git 管理下で全員共有するため不適
- **サブディレクトリ別 AGENTS.md やルール細則ファイルの生成**：本 skill のスコープ外
